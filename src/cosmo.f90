subroutine cosmo(ddCosmo, star, cart, phi, glm, psi, sigma, esolv )
   !
   !---------------------------------------------------------------------------------------
   ...
   !---------------------------------------------------------------------------------------
   !
   use ddcosmo_core, only : TddCosmo, zero, ndiis,      &
   ...
   real*8,  dimension(ddCosmo%nylm, ddCosmo%nsph), intent(in)      :: glm, psi
   real*8,  dimension(ddCosmo%nylm, ddCosmo%nsph), intent(inout)   :: sigma
   ...
   if ( .not.star ) then
      ...
      !
      if (ddCosmo%iprint.ge.4) call prtsph(ddCosmo, 'rhs of the ddCOSMO equation', ddCosmo%nsph, 0, rhs)
      !
      !       2. INITIAL GUESS
      !       ----------------
      !
      ...
      !
      call jacobi_diis(ddCosmo, ddCosmo%nsph*ddCosmo%nylm, ddCosmo%iprint, ndiis, 4, tol, rhs, sigma, n_iter, ok, lx, ldm1x, hnorm )
      !
      !       4. SOLVATION ENERGY
      !       -------------------
      !
      esolv = pt5 * ((ddCosmo%eps - one)/ddCosmo%eps) * sprod( ddCosmo%nsph*ddCosmo%nylm, sigma, psi )
      !
      !       deallocate workspace
      ...
      !
      if (ddCosmo%iprint.ge.4) call prtsph(ddCosmo, 'rhs of the ddCOSMO adjoint equation', ddCosmo%nsph, 0, psi)
      !
      !       1. INITIAL GUESS
      !       ----------------
      !
      ...
      !
      call jacobi_diis(ddCosmo, ddCosmo%nsph*ddCosmo%nylm, ddCosmo%iprint, ndiis, 4, &
         & tol, psi, sigma, n_iter, ok, lstarx, ldm1x, hnorm )
      !
   endif
   !
   !     check solution
