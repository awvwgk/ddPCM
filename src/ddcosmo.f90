...
module ddcosmo_core
   ...
   type :: TddCosmo
      ...
      !> number of openMP threads ; 0) no parallelization
...
subroutine ddinit(self, n, x, y, z, rvdw)

   type(TddCosmo), intent(inout) :: self
   integer, intent(in) :: n
   real(wp), intent(in) :: x(n), y(n), z(n), rvdw(n)
   ...
   if (self%nproc.eq.0) self%nproc = 1
   !$ call omp_set_num_threads(self%nproc)
   ...
   inear = 100000
   do i = 1, nllg
      jnear = iabs(ng0(i)-self%ngrid)
      if (jnear.lt.inear) then
         inear = jnear
         igrid = i
      end if
   end do
   self%ngrid = ng0(igrid)
   ...
   ! load a lebedev grid:
   call llgrid(self%ngrid, self%w, self%grid)
   ...
   do isph = 1, self%nsph
      ...
      do jsph = 1, self%nsph
         if (isph.ne.jsph) then
            d2 = (self%csph(1, isph) - self%csph(1, jsph))**2 &
               & + (self%csph(2, isph) - self%csph(2, jsph))**2 &
               & + (self%csph(3, isph) - self%csph(3, jsph))**2
